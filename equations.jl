...
function gce2_eqs6!(du,u,p,t)
    ...
    @inbounds for m = 0:Λ
        ...
        @inbounds for n=nmin:ny-1
            ...
            @fastmath dx[n+ny,m+1] += B[n+ny,m+1]*u.x[1][n+ny,m+1]
        ...
        @inbounds for n1=n1min:ny-1
            ...
            @inbounds for m2=0:1:min(m1,Λ-m1)
                ...
                @inbounds for n2=max(n2min,-(ny-1)-n1):min(ny-1,ny-1-n1)
                    ...
                    m::Int = m1 + m2
                    n::Int = n1 + n2
                    @fastmath dx[n+ny,m+1] += Cp[n2+ny,m2+1,n1+ny,m1+1]*u.x[1][n1+ny,m1+1]*u.x[1][n2+ny,m2+1]
        ...
        @inbounds for n1=n1min:ny-1
            @inbounds for m2=0:m1
                ...
                @inbounds for n2=max(n2min,n1-(ny-1)):min(n2max,n1+ny-1)
                    ...
                    m::Int = m1 - m2
                    n::Int = n1 - n2
                    @fastmath dx[n+ny,m+1] += Cm[n2+ny,m2+1,n1+ny,m1+1]*u.x[1][n1+ny,m1+1]*conj(u.x[1][n2+ny,m2+1])
        ...
        @inbounds for n1=-(ny-1):ny-1
            @inbounds for m2=max(Λ+1,m1-Λ):m1
                ...
                @inbounds for n2=max(-(ny-1),n1-(ny-1)):min(n2max,n1+ny-1)
                    ...
                    n::Int = n1 - n2
                    # note: u.x[2] contains H2*conj(H1) so H-H is conj(H2)*H1
                    @fastmath dx[n+ny,m+1] += Cm[n2+ny,m2+1,n1+ny,m1+1]*conj(u.x[2][n2+ny,m2-Λ,n1+ny,m1-Λ])
        ...
        @inbounds for n1=-(ny-1):ny-1
            @inbounds for m2=0:1:min(nx-1-m1,Λ)
                ...
                @inbounds for n2=max(n2min,-(ny-1)-n1):min(ny-1,ny-1-n1)
                    ...
                    m::Int = m1 + m2
                    n::Int = n1 + n2
                    @fastmath temp[n1+ny,m1-Λ,n+ny,m-Λ] += Cp[n2+ny,m2+1,n1+ny,m1+1]*u.x[1][n2+ny,m2+1]
        ...
        @inbounds for n1=-(ny-1):ny-1
            @inbounds for m2=0:1:min(Λ,m1 - Λ - 1)
                ...
                @inbounds for n2=max(n2min,n1-(ny-1)):min(ny-1,n1+ny-1)
                    ...
                    m::Int = m1 - m2
                    n::Int = n1 - n2
                    @fastmath temp[n1+ny,m1-Λ,n+ny,m-Λ] += Cm[n2+ny,m2+1,n1+ny,m1+1]*conj(u.x[1][n2+ny,m2+1])
        ...
        @inbounds for n3=-(ny-1):ny-1
            @inbounds for m=Λ+1:nx-1
                @inbounds for n=-(ny-1):ny-1
                    ...
                    @fastmath dy[n+ny,m-Λ,n3+ny,m3-Λ] += B[n+ny,m+1]*u.x[2][n+ny,m-Λ,n3+ny,m3-Λ]
                    ...
                    @inbounds for m1=max(Λ+1,m-Λ):min(nx-1,m)
                        ...
                        @inbounds for n1=max(-(ny-1),n-(ny-1)):min(n-n2min,ny-1)
                            ...
                            @fastmath accumulator += temp[n1+ny,m1-Λ,n+ny,m-Λ]*u.x[2][n1+ny,m1-Λ,n3+ny,m3-Λ]
        ...
        @inbounds for n3=-(ny-1):ny-1
            @inbounds for m=Λ+1:nx-1
                @inbounds for n=-(ny-1):ny-1
                    ...
                    @fastmath du.x[2][n+ny,m-Λ,n3+ny,m3-Λ] = dy[n+ny,m-Λ,n3+ny,m3-Λ] + conj(dy[n3+ny,m3-Λ,n+ny,m-Λ])
