...
# end
#
# function gql(lx::Float64,ly::Float64,nx::Int,ny::Int,Λ::Int,T::Float64,Ξ::Float64,β::Float64,τ::Float64,u0::Array{ComplexF64,2})
#     tspan = (0.0,T)
#     A = acoeffs(ly,ny,Ξ,τ)
...
#     prob = ODEProblem(gql_eqs!,u0,tspan,p)
#     # @time solve(prob,RK4(),adaptive=true,reltol=1e-6,abstol=1e-6,progress=true,progress_steps=1000,save_start=true,save_everystep=false,dense=false,saveat=20)
#     @time solve(prob,RK4(),dt=0.001,adaptive=false,progress=true,progress_steps=10000,save_start=true,save_everystep=false,dense=false,saveat=40)
# end
...
#     @time solve(prob,RK4(),callback=cb,tstops=poschecktimes,dt=0.005,adaptive=false,progress=true,progress_steps=2000,save_start=true,save_everystep=false,dense=false,saveat=20)
# end
#
...
function gce2(lx::Float64,ly::Float64,nx::Int,ny::Int,Λ::Int,Ξ::Float64,β::Float64,τ::Float64=0.0,νn::Float64=0.0;ic::Array{ComplexF64,2},dt::Float64=0.001,t_end::Float64=1000.0)
    A = acoeffs(ly,ny,Ξ,τ)
    B = bcoeffs(lx,ly,nx,ny,β,τ,νn)
    Cp,Cm = ccoeffs(lx,ly,nx,ny,Λ)
    p = [nx,ny,Λ,A,B,Cp,Cm]
